...
import { Loader2, ThumbsUp, ThumbsDown, Camera, Palette, Check, X } from 'lucide-react'
...
export default function ApprovalsPage() {
  ...
  const handleAction = async (type: 'price_review' | 'trait_edit', id: string, action: 'approve' | 'reject') => {
    setProcessing(id)
    try {
      const body: Record<string, unknown> = { type, id, action }
      // If there's an admin override value for trait edits
  ...
  const totalPending = pendingPriceReviews.length + pendingTraitEdits.length
    ...
    <div className="p-6 space-y-6">
      ...
      {loading ? (
        ...
        <div className="space-y-2">
          {historyTraitEdits.length === 0 && historyPriceReviews.length === 0 && (
            <div className="text-center py-12 text-gray-500 text-sm">No history yet</div>
          )}

          {[...historyTraitEdits.map(e => ({ ...e, _type: 'trait' as const })), ...historyPriceReviews.map(r => ({ ...r, _type: 'price' as const }))]
